...
/**
 * Formatea un número mexicano de 10 dígitos con espacios: 5535305000 → 55 3530 5000.
 * Para internacionales devuelve el número tal cual.
 */
function formatPhoneReadable(phone) {
  const digits = (phone || '').replace(/\D/g, '');
  const local = digits.slice(-10);
  if (digits.length <= 11 && local.length === 10) {
...
/**
 * Envía la notificación de nuevo lead al asesor por WhatsApp (con ticket y
 * comandos LISTO/REGRESA). Reutiliza el patrón de Travel.sendAdvisorNotification.
 */
async function notifyAdvisor(advisor, lead, conv, contact, reason, duplaKey, log) {
  try {
    ...
    const producto = lead.primaryProduct || 'no capturado';
...
✅ *LISTO ${ticket}* → cuando cierres/atiendas el lead (solo tracking; Ori nunca se silencia)`;
    ...
    log.info({ advisor: advisor.nombre, advisorPhone }, 'Advisor notification sent');
  } catch (error) {
